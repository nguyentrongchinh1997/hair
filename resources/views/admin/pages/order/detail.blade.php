...
<table>
    ...
    <tr class="update-customer-ajax">
            ...
            $(function(){
              ...
              $('.check-in').click(function(){
                ...
                $.get('admin/khach-hang/check/' + idOrder, function(data){
                    $('.status-ajax').html("Đã check-in");
                    $('#status' + idOrder).html("<i style='color: #007bff'>Đã check-in</i>");
                    $('.check-in').hide();
                });
              })
            })
        ...
        @if ($orderDetail->customer->full_name == '')
            <td>
                <input type="text" placeholder="Nhập tên khách hàng" class="form-control name-customer">
            </td>
            <td>
                <input type="date" class="form-control birthday" name="name_customer">
            </td>
        @else
            <td style="font-weight: bold;">
...
<center>
    <a class="btn btn-primary check-in" style="color: #fff">Check-in</a>
</center>